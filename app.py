...
async def submit_solution(
    ...
    # Execute code against associated test cases
    testcase_rows = db.query(TestCase).filter(TestCase.problem_id == problem_id).all()
    serialized_testcases = [
        {
            "input": json.loads(tc.input_data),
            "output": json.loads(tc.expected_output),
        }
        for tc in testcase_rows
    ]
    execution_result = execute_code_sync(submission.code, json.dumps(serialized_testcases))
